...
def sign_in():
    ...
    query.execute("SELECT password FROM admin WHERE username=%s", (username,))
    ...
    if result is None:
        ...
        if bcrypt.checkpw(password.encode(), stored_hashed_password.encode()):
            root.destroy()
            subprocess.run(["python", "adminMainPage.py",username,password],shell=True)
        else:
...
def open_registration():
    root.destroy() 
    subprocess.Popen(['python', 'adminRegister.py']) 
